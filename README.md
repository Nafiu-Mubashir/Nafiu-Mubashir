## 👋 Hi there, I'm Nafiu Mubashir

### 🚀 Frontend Engineer | Problem-Solver | UI Enthusiast  
[![Portfolio](https://img.shields.io/badge/🌐%20https://mubashir-lemon.vercel.app/-informational)](https://mubashir-nafiu-portfolio.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/🔗%20LinkedIn-Connect-blue)](https://www.linkedin.com/in/nafiu-mubashir/)  
[![Freelance](https://img.shields.io/badge/🚀%20Freelance-Available-brightgreen)](#)  
[![Full-time](https://img.shields.io/badge/💼%20Open%20to-Full--Time%20Roles-blueviolet)](#)  

---

### 🧠 About Me

As a passionate and results-driven **Frontend Engineer** with over **3 years of experience**, I specialize in building intuitive, high-performance web applications that enhance user experience and drive business growth.

⚙️ My expertise lies in crafting scalable and responsive UIs using modern tools like **React**, **TypeScript**, and **Tailwind CSS**.

💡 What sets me apart is my **problem-solving mindset**—I don’t just write code; I build thoughtful, elegant solutions that solve real problems and resonate with users.

🤝 Whether collaborating in cross-functional teams or working independently, I blend **creativity, technical excellence**, and **a passion for clean UX/UI** in everything I do.

🌍 I’m passionate about using technology to **make a difference** and am always open to exciting, challenging opportunities.

---

### 💼 What I Offer

- 🌟 Frontend Engineering (React, TypeScript, Tailwind CSS)
- 🧪 UI Performance Optimization
- 🎨 Design-to-Code Implementation
- ⚙️ CMS Integration (WordPress, Elementor)
- 🔧 Freelance & Full-Time Roles
- 🧑🏽‍🏫 Mentoring & Code Reviews (on request)

---

### 🛠️ Tech Stack & Tools

![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=white&style=flat)  
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white&style=flat)  
![Tailwind CSS](https://img.shields.io/badge/-TailwindCSS-38B2AC?logo=tailwind-css&logoColor=white&style=flat)  
![Next.js](https://img.shields.io/badge/-Next.js-000000?logo=nextdotjs&logoColor=white&style=flat)  
![Redux](https://img.shields.io/badge/-Redux-764ABC?logo=redux&logoColor=white&style=flat)  
![GitHub](https://img.shields.io/badge/-GitHub-181717?logo=github&logoColor=white&style=flat)  
![Figma](https://img.shields.io/badge/-Figma-F24E1E?logo=figma&logoColor=white&style=flat)  
![WordPress](https://img.shields.io/badge/-WordPress-21759B?logo=wordpress&logoColor=white&style=flat)  

---

### 🛠️ What I’m Working On
- 🧠 Building an enterprise Islamic finance app for Zakat & inheritance (Meerath)
- ✨ Open to **short-term freelance** or **solo projects** (especially frontend or WordPress)

---

### 🎯 Fun Facts

- 🎮 Gamer by night, coder by day  
- 🎬 Love movies that leave you thinking  
- 📚 Always learning something new in frontend & design  
- 💬 Favorite quote:  
  > "Great design is invisible. Great code feels like magic." ✨
